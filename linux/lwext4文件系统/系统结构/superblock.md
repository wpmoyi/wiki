[[_TOC_]]
# Super Block 超级块
- 记录文件系统的全局信息。例如块计数、inode计数、支持的功能、维护信息等。
- 位于块组0的开头，并且有多个备份，防止单一故障点损坏导致整个文件系统无法挂载。如果设置了`sparse_super`特性标志，超级块和组描述符的冗余副本仅保留在组号为0或3、5、7的幂的组中。如果未设置该标志，则在所有组中都保留冗余副本。
- 超级块校验和是根据超级块结构计算的，其中包括文件系统UUID
